...
for j in wfs:
    ...
    for s in wf_var_attr:
        if s.get('NAME').find("s_m_src_img_sor_gis") != -1:
            s['NAME'] = s['NAME'].replace(fname, ss_name)
        elif (s.get('NAME').find("Start") != -1 and iCount > 1) or  (s.get('NAME').find("$$OWN") != -1 and iCount > 1):
            continue
...
outfile_name = "wf_combined_gis_sor" + ".XML"
