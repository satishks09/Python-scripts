...
# This is a modified version of testxml5. Modified to bring transformations in the beginning followed by instances and connectors. Modified date datatype to date/time with precision 29 and scale 9
...
def gen_pkmd5(pk_attrib):
    ...
    for p in range(len(pk_attrib)):
        ...
        elif pk_attrib[p].get('DATATYPE').lower() == "num32" or pk_attrib[p].get('DATATYPE').lower() == "integer" or \
                        pk_attrib[p].get('DATATYPE').lower() == "smallint" or pk_attrib[p].get('DATATYPE').lower() == "decimal" or pk_attrib[p].get('DATATYPE').lower() == "num16" or pk_attrib[p].get('DATATYPE').lower() == "packed":
            pk_str = pk_str + "to_char(" + pk_attrib[p].get('NAME').lower() + ")"
        elif pk_attrib[p].get('DATATYPE').lower() == "date" or pk_attrib[p].get('DATATYPE').lower() == "time" or \
                        pk_attrib[p].get('DATATYPE').lower() == "timestamp":
...
outfile_name = "m_src_img_sor_gis_combined_cdc_wc" + ".XML"
