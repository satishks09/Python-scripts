...
for i in wfs:
    ...
    for s in range(len(ss_trfm_attrib)):
        if ss_trfm_attrib[s]['TRANSFORMATIONTYPE'] == 'Target Definition':
            ss_trfm_attrib[s]['SINSTANCENAME'] = "sor_" + i.lower()
            ss_trfm_attrib[s]['TRANSFORMATIONNAME'] = "sor_" + i.lower()
    ...
    for s in range(len(val)):
        if val[s]['TRANSFORMATIONTYPE'] == 'Target Definition':
            val[s]['SINSTANCENAME'] = "sor_" + i.lower()
            ...
            for j in ss_ext_attr:
                if j.get('NAME') == "Schema Name Override":
                    break
...
for j in wfs:
    ...
    for s in wf_var_attr:
        if s.get('NAME').find("s_m_src_img_sor_gis_wc") != -1:
            s['NAME'] = s['NAME'].replace(fname, ss_name)
        elif (s.get('NAME').find("Start") != -1 and iCount > 1) or  (s.get('NAME').find("$$DB2") != -1 and iCount > 1):
            continue
...
outfile_name = "wf_combined_gis_wc_sor" + ".XML"
