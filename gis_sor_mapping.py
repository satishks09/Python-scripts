...
# Src fields with hash will be replaced with underscore in target field
...
exp_name = config[sec]['exp_name']
src_db_name = config[sec]['src_db_name']
...
# Function to generate pkmd5 based on datatypes
def gen_pkmd5(pk_attrib):
    ...
    for p in range(len(pk_attrib)):
        ...
        if pk_attrib[p].get('DATATYPE').lower() == "varchar" or pk_attrib[p].get('DATATYPE').lower() == "char":
            pk_str = pk_str + "rtrim(" + pk_attrib[p].get('NAME').upper() + ")"
        elif pk_attrib[p].get('DATATYPE').lower() == "num32" or pk_attrib[p].get('DATATYPE').lower() == "integer" or pk_attrib[p].get('DATATYPE').lower() == "float" or pk_attrib[p].get('DATATYPE').lower() == "int" or pk_attrib[p].get('DATATYPE').lower() == "smallint" or pk_attrib[p].get('DATATYPE').lower() == "decimal":
            pk_str = pk_str + "to_char(" + pk_attrib[p].get('NAME').upper() + ")"
            ...
            pk_str = pk_str + "to_char(" + pk_attrib[p].get('NAME').upper() +",'YYYY-MM-DD'" + ")"
    pk_str = pk_str + "))"
    return pk_str
...
for i in sources:
    ...
    for s in src_attrib:
        ...
        scale = s.get('SCALE')
        if (datatype == "varchar" or datatype == "char" or datatype == "bit"):
            datatype = "string"
        elif datatype.lower() == "numeric":
            datatype = "decimal"
        elif datatype.lower() == "float":
            datatype = "double"
        elif datatype.lower() == "nchar":
            datatype = "nstring"
        elif datatype.lower() == "int":
            datatype = "integer"
        elif datatype.lower() == "smallint" or datatype.lower() == "tinyint":
            datatype = "small integer"
            precision = "5"
        elif datatype.lower() == "date" or datatype.lower() == "time" or datatype.lower() == "timestamp" or datatype.lower() == "datetime":
            datatype = "date/time"
            precision = "29"
            scale = "9"
        ...
        #value = value+src_name+"."+s.get('NAME')+","
        #print(value[:-1])
        ...
        if g.get('KEYTYPE') == "PRIMARY KEY":
            for j in src_attrib:
                if j.get('NAME').lower() == g.get('NAME'):
                    ...
                    getmd5_tfm_fld.set("PORTTYPE", "INPUT/OUTPUT")
    ...
    inst_src_def.set("DBDNAME", src_db_name)
    ...
    inst_exp_ctl.set("NAME", exp_name)
    inst_exp_ctl.set("REUSABLE", "YES")
    inst_exp_ctl.set("TRANSFORMATION_NAME", exp_name)
    inst_exp_ctl.set("TRANSFORMATION_TYPE", "Expression")
    inst_exp_ctl.set("TYPE", "TRANSFORMATION")

    # inst_exp_ctl = etree.SubElement(map, "INSTANCE")
    # inst_exp_ctl.set("DESCRIPTION", "")
    # inst_exp_ctl.set("NAME", "EXP_Validate_Strings")
    # inst_exp_ctl.set("REUSABLE", "YES")
    # inst_exp_ctl.set("TRANSFORMATION_NAME", "EXP_Validate_Strings")
    # inst_exp_ctl.set("TRANSFORMATION_TYPE", "Expression")
    # inst_exp_ctl.set("TYPE", "TRANSFORMATION")
    ...
    for c in tfm_val:
        if c.get('NAME') != "any":
            ...
            conn.set("FROMINSTANCE", exp_name)
    ...
    for s in src_attrib:
        ...
        if s.get('NAME').find("#") != -1:
            hash_trg = s.get('NAME').replace('#','_')
        elif s.get('NAME') == "USER":
            hash_trg = "USER_"
        else:
            hash_trg = s.get('NAME')
        conn_sq_trg.set("TOFIELD", hash_trg.lower())
    ...
    conn_sq_ctl.set("TOINSTANCE", exp_name)
    ...
    # cFlag = 0
    # for s in src_attrib:
    #     if s.get('DATATYPE') == "numeric" or s.get('DATATYPE') == "decimal":
    #         cFlag = 1
    #         from_fld = s.get('NAME')
    #         break
    # if cFlag == 0:
    ...
    # conn_sq_ctl.set("FROMINSTANCETYPE", "Source Qualifier")
    # conn_sq_ctl.set("TOFIELD", "any")
    # conn_sq_ctl.set("TOINSTANCE", "exp_ctl_columns")
    # conn_sq_ctl.set("TOINSTANCETYPE", "Expression")
...
outfile_name = "m_src_gis_sor_combined" + ".XML"
