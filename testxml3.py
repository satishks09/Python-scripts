...
#This Python Script Produces one xml per mapping for each of the source and target tables. The mapping is for IC load from Source to Target.
...
for i in sources:
    ...
    #Source Tag
    source = etree.SubElement(fold, "SOURCE", attrib=sources[cnt])
    cnt = cnt + 1

    #SourceField
    src_attrib = field_attr("SRC",src_name)
    for j in src_attrib:
        src_fld = etree.SubElement(source, "SOURCEFIELD", attrib=j)

    # Target Tag
    trg_val = read_attr("TRG", 'TARGET')
    for t in trg_val:
        if t.get('NAME') == trg_name:
            target = etree.SubElement(fold, "TARGET", attrib=t)

    #TargetField
    trg_attrib = field_attr("TRG",trg_name)
    for k in trg_attrib:
        trg_fld = etree.SubElement(target, "TARGETFIELD", attrib=k)

    # Transformation Tag
    trf_val = read_attr("TRF", 'TRANSFORMATION')
    transformation = etree.SubElement(fold, "TRANSFORMATION", attrib=trf_val[0])

    # TransformField Tag
    tfm_val = read_attr("TRF", "TRANSFORMFIELD")
    for tf in tfm_val:
        tfm_fld = etree.SubElement(transformation, "TRANSFORMFIELD", attrib=tf)

    # Mapping Tag
    map_name = 'm_src_img_sor_pms_' + src_name.lower()
    ...
    for s in src_attrib:
        ...
        sq_tfm_fld.set("PRECISION", s.get('PRECISION'))
        sq_tfm_fld.set("SCALE", s.get('SCALE'))
        ...
        if g.get('KEYTYPE') == "PRIMARY KEY":
            for j in src_attrib:
                if j.get('NAME').lower() == g.get('NAME'):
                    ...
                    getmd5_tfm_fld.set("PRECISION", j.get('PRECISION'))
                    getmd5_tfm_fld.set("SCALE", j.get('SCALE'))
    ...
    outfile_name = "m_src_img_sor_pms_" + src_name.lower() + ".XML"
